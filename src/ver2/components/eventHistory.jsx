...
function EventHistory(props) {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://14.225.7.221:8989/lovehistory/page/${count}`);
      if (!response.ok) {
      ...
      setIsLoading(false);
      console.log(jsonData.list_sukien[0].sukien)
    } catch (error) {
      console.log(error);
      setIsLoading(false);
    }
    ...
    <div className="">
      ...
      {isLoading ? (
        ...
        <div className="cursor-pointer">
                ...
                {props.data.map((array, index) => (
                  ...
                  >
                    {array.id_template === 1 && 4 ?
                      <div style={{ backgroundImage: `url(${bg1})` }} className="bg-no-repeat lg:bg-contain bg-center lg:w-full lg:h-full" >
                        <div className="grid grid-cols-2 ">
                          <div className="lg:w-[300px] flex flex-col justify-center lg:ml-28 ml-5 lg:mt-0 mt-20">
                            {/* image love */}
                            <span key={array.id} to={`/ array / ${array.id}`} className="lg:text-[24px] starborn"  >{array.ten_su_kien}</span>
                          ...
                          <div className="flex justify-center mt-16">
                            ...
                            />
                            <div style={{ backgroundImage: `url(${img1})` }} className="absolute rounded-full bg-center bg-no-repeat bg-cover lg:w-[320px] w-[120px] h-[120px] lg:h-[320px]" />
                          </div>
                        </div>
                        {/* first event */}
                      </div>
                      :
                      array.id_template === 2 ?
                        <div style={{ backgroundImage: `url(${bg2})` }} className="bg-no-repeat bg-cover rounded-[29px] h-[214px] bg-center lg:w-full lg:h-full" >

                          <div className="grid grid-cols-3">
                            <div className="flex justify-center items-center">
                              <div style={{ backgroundImage: `url(${vec1})` }} className="bg-no-repeat bg-cover lg:w-60 lg:h-60 w-32 h-32"></div>
                            </div>
                            ...
                            <div className="flex justify-center items-center">
                              <div style={{ backgroundImage: `url(${vec2})` }} className="bg-no-repeat bg-cover lg:w-60 lg:h-60 w-32 h-32"></div>
                            </div>
                          </div>
                        </div>
                        :
          ...
          <div className="pagination text-4xl flex justify-center my-6">
            <button type="button" className="py-2 px-3 bg-[#ff9f9f] rounded hover:bg-[#ff9f9f8c]" onClick={() => changePageDown()}>
            ...
            <button type="button" className="mx-3 text-white font-medium py-2 px-4 rounded bg-red-700">{count}</button>
            <button type="button" className="py-2 px-3 bg-[#ff9f9f] rounded hover:bg-[#ff9f9f8c]" onClick={() => changePageUp()}>

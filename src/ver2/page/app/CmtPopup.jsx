...
function CmtPopup(props) {
  ...
  const HandleSendCmt = async (e) => {
    ...
    const comment = { device_cmt: 'Simulator (iPhone 14 Plus)', id_toan_bo_su_kien: param.id, ipComment: '14.232.159.109', so_thu_tu_su_kien: props.data.so_thu_tu_su_kien, imageattach: imgComment ? imgComment.url : '', id_user: user.id_user, location: 'Ha Noi' }
      ...
      <div className="rounded-lg rounded-t-[36px] flex flex-col h-[95%] w-max bg-white gap-y-4">
            ...
            dataCmt.map((cmt, index) => (
              <div className="flex items-stretch gap-x-4" key={index}>
                ...
                <div className="">
                  ...
                  {cmt.imageattach ? <img className="w-[60px] h-[50px]" src={cmt.imageattach}></img> : ""}
        ...
        <div className="flex items-center justify-around mx-3 gap-x-4 rounded-full shadow-sm shadow-slate-300">
          <div className="overflow-hidden rounded-full w-[50px]">
            <img
              src={
                user.id_user ? user.link_avatar : noAvatar
              }
              alt=""
            ...
            <form onSubmit={onSubmitComment}>
              ...
              <div className="inline-block relative">
                <label for="file-input">
                  <img src="https://cdn-icons-png.flaticon.com/512/685/685655.png" width="20px" height="20px" />
                  <input type="file" onChange={onChangeImgComment} accept=".jpg" className="absolute left-0 top-0 opacity-0 w-[100%] h-[100%]" />
                </label>
              </div>
          ...
          {imgComment ? <><img className="w-[80px] h-[70px]" src='https://i.imgur.com/VLOfPdr.jpg' />

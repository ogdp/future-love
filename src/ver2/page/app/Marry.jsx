...
function Marry() {
    ...
    const fetchData = async () => {
        try {
            ...
            setTemplate("Template"+response.data.sukien[4].id_template)
            console.log("Template"+response.data.sukien[4].id_template)
        } catch (err) {
            console.log(err)
        }
    }
    ...
    return (
        <div className="flex items-center justify-center h-full">
          { data ?  < TemplateComponent data={data} stt={5}/> :  <Empty/> }
        </div>
    );
}

...
function Remarry() {
  ...
  const fetchData = async () => {
    try {
      ...
      setTemplate("Template" + response.data.sukien[6].id_template);
      console.log("Template" + response.data.sukien[6].id_template);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  return (
    <div className="flex items-center justify-center h-full">
      {data ? <TemplateComponent data={data} stt={7} /> : <Empty />}
    </div>
  );
}

export default Remarry;
